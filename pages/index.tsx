...
import { getRetoData, saveRetoData, getDiaActual, updateDia, addComidaToDia, RetoData, Comida } from '../lib/store'
...
// ── HELPERS ────────────────────────────────────────────────────
const card = (extra?: object) => ({ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, padding: 16, marginBottom: 12, ...extra })
const cardTitle = { fontSize: 9, fontWeight: 700, letterSpacing: '2.5px', color: C.muted, textTransform: 'uppercase' as const, marginBottom: 12 }
const statCard = (type?: string) => ({ background: type === 'success' ? 'rgba(0,230,118,0.06)' : type === 'warn' ? 'rgba(255,92,0,0.06)' : C.s2, border: `1px solid ${type === 'success' ? 'rgba(0,230,118,0.4)' : type === 'warn' ? C.orange : C.border}`, borderRadius: 12, padding: '12px 8px', textAlign: 'center' as const })
const bigNum = (color?: string) => ({ fontFamily: "'Bebas Neue', sans-serif", fontSize: 28, letterSpacing: 1, color: color === 'green' ? C.green : color === 'orange' ? C.orange : C.text })
const badge = (type?: string) => ({ fontSize: 9, fontWeight: 700, padding: '3px 8px', borderRadius: 6, textTransform: 'uppercase' as const, letterSpacing: 1, background: type === 'done' ? 'rgba(0,230,118,0.15)' : type === 'gym' ? 'rgba(255,92,0,0.15)' : C.dim, color: type === 'done' ? C.green : type === 'gym' ? C.orange : C.muted, border: `1px solid ${type === 'done' ? 'rgba(0,230,118,0.3)' : type === 'gym' ? 'rgba(255,92,0,0.3)' : C.border}`, display: 'inline-block' })
const btn = (primary?: boolean) => ({ padding: '14px 0', borderRadius: 14, border: primary ? 'none' : `1.5px solid ${C.border}`, background: primary ? `linear-gradient(135deg, ${C.orange}, #ff8533)` : C.dim, color: primary ? 'white' : C.muted, fontFamily: "'DM Sans', sans-serif", fontSize: 14, fontWeight: 700, cursor: 'pointer', width: '100%', boxShadow: primary ? '0 4px 20px rgba(255,92,0,0.35)' : 'none' })
const inputStyle = { width: '100%', background: C.dim, border: `1.5px solid ${C.border}`, borderRadius: 10, padding: '10px 12px', color: C.text, fontFamily: "'DM Sans', sans-serif", fontSize: 14, outline: 'none' }
...
export default function App() {
  ...
  const totalKcalHoy = dia.comidas.reduce((s, c) => s + c.kcal, 0)
  const deficit = data.objetivoKcal - totalKcalHoy
  const deficitAcumulado = data.dias.slice(0, diaActual).reduce((s, d) => s + (data.objetivoKcal - d.comidas.reduce((sc, c) => sc + c.kcal, 0)), 0)
  const diasCompletos = data.dias.filter(d => d.completo).length
  const pesoData = data.dias.filter(d => d.peso).map(d => ({ dia: `D${d.dia}`, peso: d.peso }))
  const pesoActual = [...pesoData].reverse()[0]?.peso ?? data.pesoInicial
    ...
    <div style={{ background: C.bg, minHeight: '100vh', fontFamily: "'DM Sans', sans-serif", color: C.text }}>
      <div style={{ maxWidth: 430, margin: '0 auto', paddingBottom: 90 }}>
        ...
        <nav style={{ display: 'flex', gap: 4, padding: '10px 14px', background: C.surface, borderBottom: `1px solid ${C.border}`, position: 'sticky', top: 0, zIndex: 100 }}>
          {([['hoy','🔥','HOY'],['kcal','🍽️','KCAL'],['peso','⚖️','PESO'],['reto','✅','RETO'],['reporte','📋','HOY+']] as const).map(([id, icon, label]) => (
            <button key={id} onClick={() => setTab(id)} style={{ flex: 1, padding: '8px 4px', border: 'none', borderRadius: 10, background: tab === id ? C.dim : 'transparent', color: tab === id ? C.orange : C.muted, fontFamily: 'inherit', fontSize: 10, fontWeight: 700, cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
        ...
        {tab === 'peso' && <TabPeso pesoData={pesoData} pesoInicial={data.pesoInicial} pesoActual={pesoActual} cambio={cambio} diaActual={diaActual} />}
...
// ── TAB HOY ────────────────────────────────────────────────────
function TabHoy({ dia, totalKcal, deficit, deficitAcumulado, diasCompletos, objetivo, diaActual, onReporte }: any) {
  const semana = Array.from({ length: 7 }, (_, i) => i)
  return (
    <div style={{ padding: '16px 14px' }}>
      ...
      <div style={card()}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 4 }}>
          {['L','M','X','J','V','S','D'].map((d, i) => {
            ...
            const diaInfo = diaIdx < 30 ? { num: diaIdx + 1, completo: diaIdx < diaActual - 1 } : null
            ...
            return (
              <div key={i} style={{ textAlign: 'center' }}>
                ...
                <div style={{ width: 30, height: 30, borderRadius: '50%', margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, background: diaInfo?.completo ? 'rgba(0,230,118,0.15)' : esHoy ? 'rgba(255,92,0,0.1)' : C.dim, border: `1px solid ${diaInfo?.completo ? 'rgba(0,230,118,0.4)' : esHoy ? C.orange : C.border}`, boxShadow: esHoy ? `0 0 0 2px rgba(255,92,0,0.3)` : 'none' }}>
                  {diaInfo?.completo ? '✅' : esHoy ? '🔥' : diaInfo ? '·' : ''}
                </div>
              </div>
            )
          })}
      ...
      <button onClick={onReporte} style={{ ...btn(true), marginTop: 4, fontFamily: "'Bebas Neue',sans-serif", fontSize: 18, letterSpacing: 2 }}>
        📋 HACER REPORTE
      </button>
    </div>
  )
...
// ── TAB KCAL ───────────────────────────────────────────────────
function TabKcal({ dia, totalKcal, deficit, objetivo, tdee }: any) {
    ...
    <div style={{ padding: '16px 14px' }}>
      <div style={card()}>
        ...
        <div style={{ background: C.dim, borderRadius: 99, height: 8, overflow: 'hidden', marginBottom: 8 }}>
          <div style={{ width: `${Math.max(pct, 2)}%`, height: '100%', borderRadius: 99, background: deficit >= 0 ? 'linear-gradient(90deg,#00e676,#00c853)' : 'linear-gradient(90deg,#ff3d57,#ff6b6b)', boxShadow: `0 0 10px ${deficit >= 0 ? 'rgba(0,230,118,0.4)' : 'rgba(255,61,87,0.4)'}` }} />
        </div>
      ...
      <div style={card()}>
        ...
        {dia.comidas.length === 0 && <div style={{ textAlign: 'center', padding: 16, color: C.muted, fontSize: 13 }}>Sin comidas registradas aún</div>}
          ...
          <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 0', borderBottom: i < dia.comidas.length - 1 ? `1px solid ${C.border}` : 'none' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div style={{ fontSize: 18, width: 34, height: 34, display: 'flex', alignItems: 'center', justifyContent: 'center', background: C.dim, borderRadius: 9 }}>
                {c.momento === 'Desayuno' ? '🌅' : c.momento === 'Comida' ? '🍽️' : c.momento === 'Cena' ? '🌙' : c.momento === 'Merienda' ? '🥪' : '🍎'}
              </div>
              <div>
                <div style={{ fontSize: 13, fontWeight: 600 }}>{c.momento} · {c.hora}</div>
                <div style={{ fontSize: 11, color: C.muted }}>{c.nombre}</div>
                {c.notas && <div style={{ fontSize: 10, color: C.muted, fontStyle: 'italic', marginTop: 2 }}>"{c.notas}"</div>}
              </div>
            </div>
            <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 18, color: C.orange, flexShrink: 0 }}>{c.kcal}</div>
          </div>
        ))}
      </div>

      <div style={card()}>
        <div style={cardTitle}>Tu metabolismo</div>
        {[{ icon: '🔥', name: 'TDEE estimado', sub: 'Lo que quemas al día', val: tdee.toLocaleString('es'), color: C.orange }, { icon: '🎯', name: 'Objetivo diario', sub: 'Déficit 20%', val: objetivo.toLocaleString('es'), color: C.orange }, { icon: '📉', name: 'Déficit hoy', sub: '~0.5kg grasa/semana', val: `${deficit >= 0 ? '+' : ''}${deficit.toLocaleString('es')}`, color: deficit >= 0 ? C.green : C.red }].map((r, i) => (
...
// ── TAB PESO ───────────────────────────────────────────────────
function TabPeso({ pesoData, pesoInicial, pesoActual, cambio, diaActual }: any) {
  return (
    <div style={{ padding: '16px 14px' }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, marginBottom: 12 }}>
      ...
      <div style={card()}>
        ...
        {pesoData.length < 2 ? (
          <div style={{ textAlign: 'center', padding: '24px 0', color: C.muted, fontSize: 13 }}>
            ...
            Registra más días para ver la evolución
          </div>
        ) : (
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={pesoData} margin={{ top: 5, right: 12, left: -20, bottom: 0 }}>
              ...
              <Tooltip contentStyle={{ background: C.surface, border: `1px solid ${C.orange}`, borderRadius: 8 }} labelStyle={{ color: C.orange }} itemStyle={{ color: C.text }} formatter={(v: any) => [`${v} kg`]} />
      ...
      <div style={card()}>
        <div style={cardTitle}>Historial de peso</div>
        {pesoData.length === 0 && <div style={{ textAlign: 'center', padding: 16, color: C.muted, fontSize: 13 }}>Sin datos aún</div>}
        {pesoData.map((p: any, i: number) => (
          <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: i < pesoData.length - 1 ? `1px solid ${C.border}` : 'none' }}>
            <div style={{ fontSize: 13, color: C.muted }}>{p.dia}</div>
            <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 18, color: C.orange }}>{p.peso} kg</div>
          </div>
        ))}
      </div>
    </div>
...
// ── TAB RETO ───────────────────────────────────────────────────
function TabReto({ dias, diasCompletos, diaActual }: any) {
  const sesionesGym = dias.filter((d: any) => d.gym === true).length
  ...
  return (
    <div style={{ padding: '16px 14px' }}>
      ...
      {semanas.map((semana: any[], si: number) => {
        const semanaActual = Math.floor((diaActual - 1) / 7)
        if (si > semanaActual) return (
          <div key={si} style={{ ...card(), opacity: 0.4 }}>
            <div style={cardTitle}>Semana {si + 1} — 🔒 Bloqueada</div>
            <div style={{ textAlign: 'center', padding: '12px 0', color: C.muted, fontSize: 13 }}>Completa la semana anterior para desbloquear</div>
          </div>
        )
        return (
          <div key={si} style={card()}>
            ...
            {semana.map((d: any, i: number) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 0', borderBottom: i < semana.length - 1 ? `1px solid ${C.border}` : 'none' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 22, color: d.completo ? C.green : d.dia === diaActual ? C.orange : C.muted, width: 32, letterSpacing: 1 }}>{d.dia}</div>
                  <div>
                ...
                <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                  {d.dia <= diaActual ? (<>
                    ...
                    <span style={badge()}>Pasos</span>
                    <span style={badge()}>Gym</span>
                  </>)}
                </div>
              </div>
            ))}
...
// ── TAB REPORTE ────────────────────────────────────────────────
function TabReporte({ diaActual, dia, objetivo, onUpdate }: any) {
  const [subtab, setSubtab] = useState<'actividad'|'peso'|'comida'>('actividad')
  const [pasos, setPasos] = useState(dia.pasos ?? 0)
  const [gym, setGym] = useState<boolean|null>(dia.gym)
  const [peso, setPeso] = useState(dia.peso ?? 95.0)
  ...
  return (
    <div style={{ padding: '16px 14px' }}>
      {savedMsg && (
        <div style={{ background: 'rgba(0,230,118,0.1)', border: '1px solid rgba(0,230,118,0.3)', borderRadius: 12, padding: '12px 16px', marginBottom: 12, fontSize: 14, fontWeight: 700, color: C.green, textAlign: 'center' }}>
          {savedMsg}
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: 0, marginBottom: 14 }}>
        {(['actividad','peso','comida'] as const).map((t, i) => (
          <button key={t} onClick={() => setSubtab(t)} style={{ flex: 1, padding: '10px 4px', border: `1.5px solid ${subtab === t ? C.orange : C.border}`, background: subtab === t ? C.orange : C.dim, color: subtab === t ? 'white' : C.muted, fontFamily: 'inherit', fontSize: 11, fontWeight: 700, cursor: 'pointer', borderRadius: i === 0 ? '10px 0 0 10px' : i === 2 ? '0 10px 10px 0' : 0, letterSpacing: 0.5 }}>
        ...
        <>
          ...
          <div style={card(gym !== null)}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              {[['true','💪','SÍ FUI'],['false','🛋️','NO FUI'],['null','⏳','LUEGO']].map(([val, icon, label]) => {
                const active = gym === (val === 'true' ? true : val === 'false' ? false : null)
                return (
                  <button key={val} onClick={() => setGym(val === 'true' ? true : val === 'false' ? false : null)} style={{ flex: 1, padding: '10px 4px', borderRadius: 10, border: `1.5px solid ${active ? (val === 'false' ? 'rgba(255,61,87,0.4)' : 'rgba(0,230,118,0.5)') : C.border}`, background: active ? (val === 'false' ? 'rgba(255,61,87,0.1)' : 'rgba(0,230,118,0.1)') : C.dim, color: active ? (val === 'false' ? C.red : C.green) : C.muted, fontFamily: 'inherit', fontSize: 12, fontWeight: 700, cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
          ...
          <button onClick={guardarActividad} disabled={guardando} style={{ ...btn(true), fontFamily: "'Bebas Neue',sans-serif", fontSize: 18, letterSpacing: 2 }}>
        ...
        <>
          <div style={card(true)}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <button onClick={() => setPeso(p => Math.max(30, Math.round((p - 0.1) * 10) / 10))} style={{ width: 48, height: 48, borderRadius: 12, border: `1.5px solid ${C.border}`, background: C.dim, color: C.text, fontSize: 26, cursor: 'pointer' }}>−</button>
              ...
              <button onClick={() => setPeso(p => Math.round((p + 0.1) * 10) / 10)} style={{ width: 48, height: 48, borderRadius: 12, border: `1.5px solid ${C.border}`, background: C.dim, color: C.text, fontSize: 26, cursor: 'pointer' }}>+</button>
            </div>
            <div style={{ textAlign: 'center', fontSize: 11, color: C.muted, marginTop: 8 }}>kg</div>
            <div style={{ display: 'flex', gap: 8, marginTop: 12, justifyContent: 'center' }}>
              {[-1, -0.5, 0.5, 1].map(d => (
                <button key={d} onClick={() => setPeso(p => Math.max(30, Math.round((p + d) * 10) / 10))} style={{ background: C.dim, border: `1px solid ${C.border}`, color: C.muted, borderRadius: 8, padding: '6px 12px', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}>{d > 0 ? `+${d}` : d}</button>
              ))}
            </div>
          </div>

          <button onClick={guardarPeso} disabled={guardando} style={{ ...btn(true), fontFamily: "'Bebas Neue',sans-serif", fontSize: 18, letterSpacing: 2 }}>
      ...
      {subtab === 'comida' && (
        <>
          <div style={card()}>
            ...
            <div onClick={() => fileRef.current?.click()} style={{ border: `2px dashed ${C.border}`, borderRadius: 14, padding: fotoPreview ? 0 : 24, textAlign: 'center', cursor: 'pointer', overflow: 'hidden', background: C.dim, transition: 'all 0.2s' }}>
              {fotoPreview ? (
                <img src={fotoPreview} style={{ width: '100%', maxHeight: 220, objectFit: 'cover', display: 'block' }} />
              ) : (
                <>
                  <div style={{ fontSize: 32, marginBottom: 6 }}>📷</div>
                  <div style={{ fontSize: 13, fontWeight: 600, color: C.muted }}>Toca para añadir foto</div>
                  <div style={{ fontSize: 11, color: C.muted, marginTop: 3, opacity: 0.6 }}>Cámara o galería · IA analizará las calorías</div>
                </>
              )}
            </div>
            <input ref={fileRef} type="file" accept="image/*" capture="environment" onChange={handleFoto} style={{ display: 'none' }} />
            {fotoPreview && (
              <button onClick={() => { setFotoPreview(null); setAnalisisResult(null); }} style={{ marginTop: 8, width: '100%', padding: '8px', borderRadius: 10, border: `1px solid ${C.border}`, background: 'transparent', color: C.muted, fontFamily: 'inherit', fontSize: 12, cursor: 'pointer' }}>
                🗑️ Cambiar foto
              </button>
            )}
          </div>
          ...
          {analizando && (
            <div style={{ ...card(), textAlign: 'center', padding: 24 }}>
              ...
              <div style={{ fontSize: 12, color: C.muted, marginTop: 4 }}>Identificando ingredientes y calorías</div>
            </div>
          )}

          {analisisResult && !analizando && (
            <div style={{ ...card(true), background: 'rgba(0,230,118,0.04)' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 8, marginBottom: 12 }}>
                {[['Kcal', analisisResult.kcal, C.orange],['Prot.', `${analisisResult.proteinas}g`, C.green],['Carbs', `${analisisResult.carbohidratos}g`, '#64b5f6'],['Grasas', `${analisisResult.grasas}g`, '#ffb74d']].map(([l,v,c]: any) => (
              ...
              {analisisResult.notas && <div style={{ fontSize: 11, color: C.muted, fontStyle: 'italic', marginBottom: 10 }}>💬 {analisisResult.notas}</div>}
            </div>
          )}

          <div style={card()}>
            <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 14 }}>Detalles de la comida</div>
            {[
              ['Momento', <select value={mealForm.momento} onChange={e => setMealForm({...mealForm, momento: e.target.value})} style={{ ...inputStyle, color: C.text }}>{['Desayuno','Almuerzo','Comida','Merienda','Cena','Snack'].map(m => <option key={m} style={{ background: C.surface }}>{m}</option>)}</select>],
              ['¿Qué comiste?', <input value={mealForm.nombre} onChange={e => setMealForm({...mealForm, nombre: e.target.value})} placeholder="Ej: Pollo con arroz y ensalada" style={inputStyle} />],
              ['Kcal', <input type="number" value={mealForm.kcal} onChange={e => setMealForm({...mealForm, kcal: e.target.value})} placeholder="Se rellena automáticamente con la foto" style={inputStyle} />],
              ['Notas (opcional)', <input value={mealForm.notas} onChange={e => setMealForm({...mealForm, notas: e.target.value})} placeholder="Ingredientes extra, detalles..." style={inputStyle} />],
            ].map(([label, input]: any, i) => (
          ...
          <button onClick={guardarComida} disabled={guardando || !mealForm.nombre} style={{ ...btn(true), fontFamily: "'Bebas Neue',sans-serif", fontSize: 18, letterSpacing: 2, opacity: !mealForm.nombre ? 0.5 : 1 }}>
